...
import { FaSearch, FaTh, FaList, FaGenderless } from 'react-icons/fa';
...
const UnisexCollection = () => {
  ...
  const fetchUnisexProducts = async () => {
    const cachedProducts = cache.get('unisex-products');
    if (cachedProducts) {
    ...
    try {
      ...
      const unisexProducts = Array.isArray(data) ? data.filter(product => 
        product.category?.toLowerCase() === 'unisex' || 
        product.gender?.toLowerCase() === 'unisex'
      ) : [];
      
      setProducts(unisexProducts);
      setFilteredProducts(unisexProducts);
      cache.set('unisex-products', unisexProducts);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-silver-light to-beige-light">
      <div className="pt-20">
        <div className="gradient-silver text-charcoal shadow-sm border-b mb-8">
              ...
              <div>
                <div className="flex items-center space-x-3 sm:space-x-4 mb-4">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 gradient-gold rounded-2xl flex items-center justify-center">
                    <FaGenderless className="text-charcoal text-xl sm:text-2xl" />
                  </div>
                  <div>
                    <span className="text-gold font-medium tracking-wider text-xs sm:text-sm uppercase block">Unisex Perfumes</span>
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-charcoal">Perfumes for Everyone</h1>
                  </div>
                </div>
                <p className="text-charcoal-light text-sm sm:text-base lg:text-lg max-w-2xl">Great perfumes that work for both men and women, perfect for anyone who loves quality scents.</p>
              </div>
          ...
          <div className="card-elegant p-4 sm:p-6 lg:p-8 mb-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              >
                ...
                <option value="price-low">Price: Accessible to Exclusive</option>
                <option value="price-high">Price: Exclusive to Accessible</option>
            ...
            <div className="text-center py-20">
              <div className="max-w-md mx-auto">
                <div className="w-24 h-24 bg-gold/20 rounded-full flex items-center justify-center mx-auto mb-6">
                  <FaGenderless className="text-gold text-3xl" />
                </div>
                <h3 className="text-2xl font-bold text-charcoal mb-4">No unisex perfumes found</h3>
